...
import { Ref } from "vue";
import { edges, nodes } from "/@/assets/tree-graph/data.json";
...
function drawTree() {
  ...
  function createNode(...anotherNode) {
    nodes_data = [...nodes_data, ...anotherNode];
  ...
  function createRelation(...anotherRelation) {
    edges_data = [...edges_data, ...anotherRelation];
  ...
  function creatForceSimulation() {
    let _forceSimulation = d3
      .forceSimulation(nodes_data)
      .force("charge", d3.forceManyBody())
      .force("center", d3.forceCenter(width.value / 2, height.value / 2))
      .force("link", d3.forceLink(relationEdges).distance(150));

    function linksTick() {
      let curNode, targetNode;
      treeNode //设置中心
        .attr("transform", (d) => {
          curNode = nodes_data.find((item) => item.name === d.name) ?? { x: 0, y: 0 };
      ...
      relationEdges
        .attr("x1", (d) => {
          curNode = nodes_data.find((item) => item.name === d.source) ?? { x: 0, y: 0 };
          return curNode.x;
        })
        .attr("y1", (d) => {
          curNode = nodes_data.find((item) => item.name === d.source) ?? { x: 0, y: 0 };
        ...
        .attr("x2", (d) => {
          targetNode = nodes_data.find((item) => item.name === d.target) ?? { x: 0, y: 0 };
  ...
  function addNode(...another) {
    ...
    treeNode._groups[0].push(...newNodes._groups[0]);
  }
  ...
  function attachSimulation() {
    forceSimulation.nodes(nodes_data)
      .force("charge", d3.forceManyBody())
      .force("center", d3.forceCenter(width.value / 2, height.value / 2))
      .force("link").links(relationEdges);
  ...
  return {
    ...
    relationEdges,
    initBinaryTreeGroup,
    createNode: (...nodes_data) => (treeNode = createNode(...nodes_data)),
    createRelation: (...edges_data) => (relationEdges = createRelation(...edges_data)),
    createListener,
    creatForceSimulation: () => (forceSimulation = creatForceSimulation()),
    addNode,
    addRelation,
    attachSimulation
  };
