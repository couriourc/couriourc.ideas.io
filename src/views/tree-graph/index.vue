...
<template>
  <div>
    ...
    <svg id="svg" width="1200" height="750"></svg>
    <!-- 绘制图例 -->
    <div id="indicator">
      <!-- 利用 item 遍历一个数组 利用 index 遍历另外一个数组 -->
      <div v-for="(name, index) in names" :key="index">
        <span
            ...
            :style="{
            backgroundColor: states[index] === 'on' ? colors[index] : '#aaa',
          }"
        ></span>
        {{ name }}
      </div>
...
const props = defineProps({
  ...
  // 自定义图例（数组保证一一对应）
  // names		图例名称变量制作图标
  // labels		节点的标签名称（与 records.json 中保证相同）
  names: {
    type: Array,
  },
  labels: Array,
...
watch(props.data, () => {
  ...
  const svgDom = ref(null)// svg 的 DOM 元素 => d3.select('#svg1')
  const keywords = ref("")
  const nodeState = ref(0)
  // 文本状态，表示是否显示文本信息（0：显示 /1：不显示）
  const textState = ref(0)
  ...
  const states = ref([])
  const selectNodeData = ref({}) // 选中节点的详细信息展 )
  const isNodeClicked = ref(false)// 是否点击（选中）节点
  // 用于位置、大小矫正（暂不使用）
  ...
  const menu = ref([
    ...
    },
    {
      ...
      action: (elm, d) => {
        console.log(d);
        // 遍历保留对应节点
        nodes.value = props.data.nodes.filter((node) => {
          if (node.id === d.id) return true;
          else {
            for (let i = 0; i < data.value.links.length; i++) {
              // 如果 links 的起点等于 name，并且终点等于正在处理的则显示
              if (
                  props.data.value.links[i].source.id === node.id &&
                  props.data.value.links[i].target.id === d.id
              ) {
                return true;
              }
              if (
                  props.data.value.links[i].target.id === node.id &&
                  props.data.value.links[i].source.id === d.id
              ) {
                return true;
              }
            }
        ...
        // 遍历保留节点的关联关系
        links.value = props.data.links.filter((link) => {
          return link.source.id === d.id || link.target.id === d.id;
        });
        d3render(); // 重新渲染图
        stateInit();
  ...
  const temp = ref({}) // 临时存储编辑时的节点信 )
  const dialogFormVisible = ref(false)
  const isEdit = ref(true)
  // 节点属性对应的标签名称
  const nodeObjMap = ref({
    address: " 注册地址 ",
    captial: " 注册资本 ",
    credit_code: " 信用代码 ",
    name: " 节点名称 ",
  ...
  function btnEdit() {
    this.temp = Object.assign({}, selectNodeData.value.properties); // copy obj
  ...
  function doEdit() {
    ...
    let i = 0;
    // 更新 props 的 data 和 selectNodeData
    selectNodeData.value.name = temp.value.name;
    selectNodeData.value.properties = temp.value.temp;
    for (let node of props.data.nodes) {
      ...
      if (node.id === this.selectNodeData.id) {
        ...
        props.data.nodes[i].properties = this.temp;
        nodes.value[i].properties = this.temp;
        break;
      }
      i++;
    }
...
// 搜索包含关键字的节点
  function searchKeyWords(value) {
    // 如果 Input 值是空的显示所有的圆和线 (没有进行筛选)
    if (this.keywords === "") {
      this.clearGraphStyle();
    }
    // 否则判断判断三个元素是否等于 name 值，等于则显示该值
    else {
      ...
      this.svgDom
          ...
          .selectAll("circle")
          .attr("class", (d) => {
            // 输入节点 id 的小写等于 name 则显示，否则隐藏
            if (d.properties.name.indexOf(name) >= 0) {
              return "fixed";
            } else {
              // 优化：与该搜索节点相关联的节点均显示
              // links 链接的起始节点进行判断, 如果其 id 等于 name 则显示这类节点
              // 注意: graph=data
              for (var i = 0; i < this.links.length; i++) {
                ...
                // 如果 links 的终点等于 name，并且起点等于正在处理的则显示
                if (
...
// d3 初始化，包括数据解析、数据渲染
  function d3init() {
    ...
    this.svgDom = d3.select("#svg"); // 获取 svg 的 DOM 元素
...
// 数据状态初始化
  ...
  function d3render() {
    var _this = this; // 临时获取 Vue 实例，避免与 d3 的 this 指针冲突
    ...
    // 解析 json 和数据处理
    // 现在解析 json 直接在 d3jsonParser() 中更新 nodes 和 links
    ...
    // 利用 d3.forceSimulation() 定义关系图 包括设置边 link、排斥电荷 charge、关系图中心点
    var simulation = d3
    ...
    // D3 映射数据至 HTML 中
    // g 用于绘制所有边,selectALL 选中所有的 line, 并绑定数据 data(graph.links),enter().append("line") 添加元素
    // 数据驱动文档, 设置边的粗细
    var link = svg
        .append("g")
        .attr("class", "links")
    ...
    // 添加所有的点
    // selectAll("circle") 选中所有的圆并绑定数据, 圆的直径为 d.size
    // 再定义圆的填充色, 同样数据驱动样式, 圆没有描边, 圆的名字为 d.id
    ...
    var node = svg
        ...
        .on("mouseenter", function (event) {
          ...
          // 设置 #info h4 样式的颜色为该节点的颜色，文本为该节点 name
    ...
    // simulation 中 ticked 数据初始化并生成图形
     ...
     */
    // ticked() 函数确定 link 线的起始点 x、y 坐标 node 确定中心点 文本通过 translate 平移变化
    function ticked() {
      link
    ...
    /**
     * 文本分隔（根据字数在当前选择器中分隔三行，超过 10 字省略）
     * @method textBreaking
     * @param {d3text} 文本对应的 DOM 对象
     * @param {text} 节点名称的文本值
     * @return {void}
     */
    ...
    // 分别定义节点和文本的点击事件
    // 优化：由于点击前必定触发 mouseenter 事件，所以不用再去查找节点 id
    //      直接根据 this.selectNodeData 拿到节点信息
    // 优化后：只需定义一个点击事件即可
    function nodeClick(event, d) {
      // console.log('node clicked!')
      // sticked 用于固定节点（无法实现节点固定功能）
      // delete d.fx
      // delete d.fy
      ...
      // 直接通过 this.selectNodeData 拿到节点信息
...
// 根据当前节点名称来更改图样式
  function changeGraphStyle(name) {
    // console.log(this.isNodeClicked)
    // 选择 #svg1 .nodes 中所有的 circle，再增加个 class
    this.svgDom
        .select(".nodes")
        .selectAll("circle")
        .attr("class", (d) => {
          // 节点属性 name 是否等于 name，返回 fixed（激活选中样式）
          if (d.properties.name == name) {
            return "fixed";
          }
          // 当前节点返回空，否则其他节点循环判断是否被隐藏起来 (CSS 设置隐藏)
          else {
            // links 链接的起始节点进行判断, 如果其 id 等于 name 则显示这类节点
            // 注意: graph = data
            for (var i = 0; i < this.links.length; i++) {
