 ...
 * @author couriourc
 */
type CachedId = string | number
interface CachedPromiseInterface {
  cache: any
  get: (id: string) => any
  set(): Promise<any>
  delete(): Promise<Array<CachedId>>
}

export default class CachedPromise implements CachedPromiseInterface {
  private readonly cache: { [key: CachedId]: Promise<any> } = {}
   ...
   */
  delete(deleteIds: CachedId | Array<CachedId>) {
    let requestIds = []
